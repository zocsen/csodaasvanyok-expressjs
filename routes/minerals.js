...
router.get("/:id", cacheMiddleware(2000000), async (req, res) => {
  ...
  try {
    ...
    if (!mineral) {
      res
        .status(404)
        .json({ message: "The mineral with the given ID was not found." });
    }
...
router.post("/", async (req, res) => {
  try {
    if (!req.body.name) {
      return res
        .status(400)
        .json({ success: false, message: "Mineral name is required" });
    }
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (typeof benefitString !== "string") {
      return res
        .status(400)
        .json({ message: "The benefit field must be a string." });
    }
    const benefitArray = benefitString.split(",").map((id) => new ObjectId(id));
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!mineral) {
      return res
        .status(404)
        .json({ success: false, message: "Mineral not found!" });
    }
    clearAllCache();
